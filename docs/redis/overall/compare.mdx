...
- **Complex Pricing:** FaunaDB has a complicated pricing. It has 6 different
  ...
  [this article](https://docs.fauna.com/fauna/current/concepts/billing) where
  the pricing is explained. If your use case is write heavy and if your requests
  have bigger payloads, then it can become expensive very easily. On the other
  hand, Upstash has different options for different needs and pricing is simple
  for all options. You pay per request in addition to storage cost which is
  generally much smaller amount.
