...
title: '"SolidJS"'
---
This guide provides detailed, step-by-step instructions on how to use and deploy Upstash Workflows with SolidJS. You can also explore our [SolidJS example](https://github.com/upstash/workflow-js/tree/main/examples/solidjs) for a detailed, end-to-end example and best practices.
## Prerequisites
...
After setting your live URL as the environment variable or `baseUrl` option, trigger your workflow by first starting your SolidJS app:
```bash Terminal
npm run dev
```
...
2. **Remove Local Development Settings**: In your production code, you can remove or conditionally exclude any local development settings. For example, the `baseUrl` option in the `serve` function can be omitted in production:
   ...
   export const { POST } = serve(
     ...
     },
     {
       ...
       baseUrl: process.env.NODE_ENV === "development" ? "https://<YOUR_NGROK_OR_TUNNEL_URL>/" : undefined,
     }
   )
   ```
...
2. Explore our [SolidJS example](https://github.com/upstash/workflow-js/tree/main/examples/solidjs) for a detailed, end-to-end example and best practices.
